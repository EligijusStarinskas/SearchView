...
public class SearchView extends FrameLayout implements Filter.FilterListener {
        ...
        switch (mVersion) {
            case SearchCodes.VERSION_TOOLBAR_MIXED:
                if (!isSearchOpen()) {
                    ...
                    List<Animator> animators = SearchAnimator.getCardWithMarginsMarginsAnimators(mCardView,
                            mCardView.getHeight() + top, mCardView.getWidth() + leftStart + rightEnd, top, 0, leftStart, 0);
    ...
    private boolean isSuggestionsAllowed() {
        return mVersion != SearchCodes.VERSION_TOOLBAR_MIXED &&
                mSearchAdapter != null && mSearchAdapter.getItemCount() > 0
    ...
    private void collapseViewIfNeeded() {
        if (mVersion == SearchCodes.VERSION_TOOLBAR_MIXED && isSearchOpen()) {
            ...
            List<Animator> animators = SearchAnimator.getCardWithMarginsMarginsAnimators(mCardView,
                    mCardView.getHeight() - top, mCardView.getWidth() - leftStart - rightEnd, 0, top, 0, leftStart);
