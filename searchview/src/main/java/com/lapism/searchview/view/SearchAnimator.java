...
class SearchAnimator {
	...
	@TargetApi(Build.VERSION_CODES.LOLLIPOP) public static void revealInAnimation(
			final Context mContext, final View animatedView, final int startCy, int duration) {

		int cx = animatedView.getWidth() - mContext.getResources()
				.getDimensionPixelSize(R.dimen.search_key_line);
		int cy = startCy == -1
				? animatedView.getHeight() / 2
				: startCy;
	...
	@TargetApi(Build.VERSION_CODES.LOLLIPOP) public static void revealOutAnimation(
			final Context mContext, final View animatedView, final int endCy, int duration) {
	...
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH) public static void fadeInAnimation(
	...
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH) public static void fadeOutAnimation(
			final View view, int duration) {
	...
	public static List<Animator> getCardWithMarginsMarginsAnimators(
			final CardView view, int toHeight, int toWidth, int fromMarginTop, int toMarginTop, int fromMarginLeftRight,
			int toMarginLeftRight) {

		List<Animator> animatorList = new ArrayList<>();
		animatorList.add(getHeightMarginAnimator(fromMarginTop, toMarginTop, view));
	...
	private static Animator getHeightMarginAnimator(int fromValue, int toValue, final CardView view) {
		...
		animatorHeightMargins.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			@Override public void onAnimationUpdate(ValueAnimator valueAnimator) {
				...
				if (params != null) {
					...
					params.bottomMargin = (int) (Integer) valueAnimator.getAnimatedValue();
					view.setLayoutParams(params);
				}
			}
		});
	...
	public static void runAnimatorSet(
			List<Animator> animatorList, int duration, Animator.AnimatorListener animatorListener) {
